...
Error* cff_read_offset(
    ...
    TRY(parse_ctx_bound_check(ctx, offset_size));

    *offset_out = 0;
    for (size_t idx = 0; idx < offset_size; idx++) {
        *offset_out <<= 8;
        *offset_out |= (CffOffset)ctx->buffer[ctx->offset++];
    }
...
Error* cff_read_offset_size(ParseCtx* ctx, CffOffsetSize* offset_size_out) {
    ...
    TRY(parse_ctx_read_u8(ctx, offset_size_out));
...
Error* cff_read_sid(ParseCtx* ctx, CffSID* sid_out) {
    ...
    uint16_t card16;
    TRY(parse_ctx_read_u16_be(ctx, &card16));
...
static Error*
read_int_operand(ParseCtx* ctx, uint8_t byte0, CffToken* token_out) {
    RELEASE_ASSERT(ctx);
    ...
    uint8_t byte1;
    TRY(parse_ctx_read_u8(ctx, &byte1));
    ...
    uint8_t byte2;
    TRY(parse_ctx_read_u8(ctx, &byte2));
    if (byte0 == 28) {
    ...
    uint8_t byte3;
    uint8_t byte4;
    TRY(parse_ctx_read_u8(ctx, &byte3));
    TRY(parse_ctx_read_u8(ctx, &byte4));
...
static Error* read_real_operand(ParseCtx* ctx, CffToken* token_out) {
    RELEASE_ASSERT(ctx);
    RELEASE_ASSERT(token_out);
    ...
    while (true) {
        uint8_t nibble = 0;
        if (high_nibble) {
            TRY(parse_ctx_read_u8(ctx, &current_byte));
...
Error* cff_read_token(ParseCtx* ctx, CffToken* token_out) {
    ...
    uint8_t byte0;
    TRY(parse_ctx_read_u8(ctx, &byte0));

    if (byte0 <= 21) {
        token_out->type = CFF_TOKEN_OPERATOR;
        token_out->value.operator = byte0;
    } else if (byte0 == 30) {
        TRY(read_real_operand(ctx, token_out));
        ...
        TRY(read_int_operand(ctx, byte0, token_out));
    }
...
Error* cff_get_str(Arena* arena, ParseCtx* ctx, size_t length, char** str_out) {
    ...
    TRY(parse_ctx_bound_check(ctx, length));

    *str_out = arena_alloc(arena, sizeof(char) * (length + 1));
    memcpy(*str_out, ctx->buffer + ctx->offset, length);
    (*str_out)[length] = '\0';
    ctx->offset += length;
...
TEST_FUNC(test_cff_read_offset) {
    uint8_t buffer[] = {0xa2, 0x2f, 0xe6, 0xf6, 0x42};
    ParseCtx ctx = parse_ctx_new(buffer, sizeof(buffer) / sizeof(uint8_t));

    CffOffset offset;
    TEST_REQUIRE(parse_ctx_seek(&ctx, 2));
    TEST_REQUIRE(cff_read_offset(&ctx, 1, &offset));
    TEST_ASSERT_EQ((CffOffset)0xe6, offset);

    TEST_REQUIRE(cff_read_offset(&ctx, 2, &offset));
    TEST_ASSERT_EQ((CffOffset)0xf642, offset);

    TEST_REQUIRE(parse_ctx_seek(&ctx, 1));
    TEST_REQUIRE(cff_read_offset(&ctx, 3, &offset));
    TEST_ASSERT_EQ((CffOffset)0x2fe6f6, offset);

    TEST_REQUIRE(parse_ctx_seek(&ctx, 0));
    TEST_REQUIRE(cff_read_offset(&ctx, 4, &offset));
    TEST_ASSERT_EQ((CffOffset)0xa22fe6f6, offset);

    return TEST_RESULT_PASS;
}

TEST_FUNC(test_cff_read_offset_eof) {
    uint8_t buffer[] = {0xa2, 0x2f, 0xe6, 0xf6, 0x42};
    ParseCtx ctx = parse_ctx_new(buffer, sizeof(buffer) / sizeof(uint8_t));

    CffOffset offset;
    TEST_REQUIRE(parse_ctx_seek(&ctx, 2));
    TEST_REQUIRE_ERR(cff_read_offset(&ctx, 4, &offset), CTX_EOF);

    return TEST_RESULT_PASS;
}

TEST_FUNC(test_cff_read_offset_size) {
    uint8_t buffer[] = {0x1, 0x2, 0x3, 0x4};
    ParseCtx ctx = parse_ctx_new(buffer, sizeof(buffer) / sizeof(uint8_t));

    CffOffsetSize offset_size;
    TEST_REQUIRE(cff_read_offset_size(&ctx, &offset_size));
...
TEST_FUNC(test_cff_read_offset_size0) {
    uint8_t buffer[] = {0x0};
    ParseCtx ctx = parse_ctx_new(buffer, sizeof(buffer) / sizeof(uint8_t));

    CffOffsetSize offset_size;
    TEST_REQUIRE_ERR(
        cff_read_offset_size(&ctx, &offset_size),
        CFF_ERR_INVALID_OFFSET_SIZE
    );
...
TEST_FUNC(test_cff_read_offset_size5) {
    uint8_t buffer[] = {0x5};
    ParseCtx ctx = parse_ctx_new(buffer, sizeof(buffer) / sizeof(uint8_t));
...
TEST_FUNC(test_cff_read_sid) {
    uint8_t buffer[] = {0x95, 0x5c, 0xd5, 0xc3};
    ParseCtx ctx = parse_ctx_new(buffer, sizeof(buffer) / sizeof(uint8_t));

    CffSID sid;
    TEST_REQUIRE(cff_read_sid(&ctx, &sid));
...
TEST_FUNC(test_cff_read_sid_64999) {
...
TEST_FUNC(test_cff_read_sid_invalid_65000) {
    uint8_t buffer[] = {0xfd, 0xe8};
    ParseCtx ctx = parse_ctx_new(buffer, sizeof(buffer) / sizeof(uint8_t));

    CffSID sid;
    TEST_REQUIRE_ERR(cff_read_sid(&ctx, &sid), CFF_ERR_INVALID_SID);

    return TEST_RESULT_PASS;
}

TEST_FUNC(test_cff_read_sid_eof) {
    uint8_t buffer[] = {0xfd};
    ParseCtx ctx = parse_ctx_new(buffer, sizeof(buffer) / sizeof(uint8_t));

    CffSID sid;
    TEST_REQUIRE_ERR(cff_read_sid(&ctx, &sid), CTX_EOF);

    return TEST_RESULT_PASS;
}

TEST_FUNC(test_cff_read_operator) {
    for (uint8_t operator = 0; operator <= 21; operator++) {
        uint8_t buffer[] = {operator};
        ParseCtx ctx = parse_ctx_new(buffer, sizeof(buffer) / sizeof(uint8_t));

        CffToken token;
        TEST_REQUIRE(cff_read_token(&ctx, &token));
...
TEST_FUNC(test_cff_read_token_eof) {
    uint8_t buffer[] = {0};
    ParseCtx ctx = parse_ctx_new(buffer, 0);

    CffToken token;
    TEST_REQUIRE_ERR(cff_read_token(&ctx, &token), CTX_EOF);

    return TEST_RESULT_PASS;
}

TEST_FUNC(test_cff_read_token_reserved) {
    ...
    for (size_t idx = 0; idx < sizeof(reserved_bytes) / sizeof(uint8_t);
        ...
        CffToken token;
        TEST_REQUIRE_ERR(cff_read_token(&ctx, &token), CFF_ERR_RESERVED);
    }
...
TEST_FUNC(test_cff_read_int_operand) {
...
TEST_FUNC(test_cff_read_real_operand) {
    uint8_t buffer[] =
...
TEST_FUNC(test_cff_read_real_operand_no_fractional) {
...
TEST_FUNC(test_cff_read_real_operand_no_fractional_with_exp) {
...
TEST_FUNC(test_cff_read_real_operand_no_integer) {
...
TEST_FUNC(test_cff_read_real_operand_fractional_exp_err) {
    uint8_t buffer[] = {0x1e, 0x5b, 0x2a, 0x5f};
    ParseCtx ctx = parse_ctx_new(buffer, sizeof(buffer) / sizeof(uint8_t));

    CffToken token;
    TEST_REQUIRE_ERR(
        cff_read_token(&ctx, &token),
        CFF_ERR_INVALID_REAL_OPERAND
    );
...
TEST_FUNC(test_cff_read_real_operand_trailing_zeros) {
...
TEST_FUNC(test_cff_read_real_operand_reserved_nibble) {
