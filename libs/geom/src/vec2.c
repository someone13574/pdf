...
GeomVec2 geom_vec2_normalize(GeomVec2 vec) {
    double length_squared = geom_vec2_len_sq(vec);
    if (length_squared < 1e-24) {
        return geom_vec2_new(0.0, 0.0);
    }

    return geom_vec2_scale(vec, 1.0 / sqrt(length_squared));
}
...
TEST_FUNC(test_geom_vec2_lerp_and_len_sq) {
    GeomVec2 mid =
        geom_vec2_lerp(geom_vec2_new(0.0, 0.0), geom_vec2_new(2.0, 4.0), 0.5);
    TEST_ASSERT_EQ(mid.x, 1.0);
    TEST_ASSERT_EQ(mid.y, 2.0);

    GeomVec2 delta =
        geom_vec2_sub(geom_vec2_new(5.0, 6.0), geom_vec2_new(1.0, 2.0));
    TEST_ASSERT_EQ(geom_vec2_len_sq(delta), 32.0);

    return TEST_RESULT_PASS;
}

TEST_FUNC(test_geom_vec2_cross_perpendicular_normalize) {
    TEST_ASSERT_EQ(
        geom_vec2_cross(geom_vec2_new(1.0, 0.0), geom_vec2_new(0.0, 1.0)),
        1.0
    );
    TEST_ASSERT_EQ(
        geom_vec2_cross(geom_vec2_new(0.0, 1.0), geom_vec2_new(1.0, 0.0)),
        -1.0
    );

    GeomVec2 perpendicular = geom_vec2_perpendicular(geom_vec2_new(2.0, 3.0));
    TEST_ASSERT_EQ(perpendicular.x, -3.0);
    TEST_ASSERT_EQ(perpendicular.y, 2.0);

    GeomVec2 normalized = geom_vec2_normalize(geom_vec2_new(3.0, 4.0));
    TEST_ASSERT(fabs(normalized.x - 0.6) < 1e-12);
    TEST_ASSERT(fabs(normalized.y - 0.8) < 1e-12);

    GeomVec2 normalized_zero = geom_vec2_normalize(geom_vec2_new(0.0, 0.0));
    TEST_ASSERT_EQ(normalized_zero.x, 0.0);
    TEST_ASSERT_EQ(normalized_zero.y, 0.0);

    TEST_ASSERT(
        fabs(
            geom_vec2_angle(geom_vec2_new(0.0, 0.0), geom_vec2_new(0.0, 1.0))
            - M_PI_2
        )
        < 1e-12
    );
