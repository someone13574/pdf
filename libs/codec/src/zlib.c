...
static Error* zlib_decode_cmf(BitStream* bitstream, ZlibCMF* cmf_out) {
    RELEASE_ASSERT(bitstream);
    RELEASE_ASSERT(cmf_out);

    TRY(bitstream_read_n(bitstream, 4, &cmf_out->method));
    TRY(bitstream_read_n(bitstream, 4, &cmf_out->info));
    ...
    switch (cmf_out->method) {
        ...
        case 15: {
            return ERROR(CODEC_ERR_ZLIB_RESERVED_CM);
        }
        default: {
            return ERROR(CODEC_ERR_ZLIB_INVALID_CM);
        }
    }
...
static Error* zlib_decode_flg(BitStream* bitstream, ZlibFLG* flg_out) {
    RELEASE_ASSERT(bitstream);
    RELEASE_ASSERT(flg_out);

    TRY(bitstream_read_n(bitstream, 5, &flg_out->check));
    TRY(bitstream_next(bitstream, &flg_out->dict_enable));
    TRY(bitstream_read_n(bitstream, 2, &flg_out->level));

    if (flg_out->dict_enable != 0) {
        // Adler32 of the preset dictionary
        TRY(bitstream_read_n(bitstream, 32, &flg_out->dict));
    } else {
        flg_out->dict = 0;
    }
...
Error* decode_zlib_data(
    ...
    // Read header
    ZlibCMF cmf;
    TRY(zlib_decode_cmf(&bitstream, &cmf));
    // size_t window_size = 1 << (cmf.info + 8);

    ZlibFLG flg;
    TRY(zlib_decode_flg(&bitstream, &flg));
    ...
    // Deflate stream
    TRY(decode_deflate_data(arena, &bitstream, decoded_bytes));
    ...
    TRY(bitstream_read_n(&bitstream, 32, &stored_checksum));
